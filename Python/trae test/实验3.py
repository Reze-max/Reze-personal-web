...
if user_turns >= 3:
    with st.container(key="export_container"):
        st.markdown("""
        <style>
        [data-testid="stHorizontalBlock"] .st-key-export_container {
            position: sticky;
            top: 0;
            background: white;
            padding: 10px 0;
        ...
        if st.button("✅ 完成并导出实验数据") or st.session_state.experiment_completed:
            ...
            if st.session_state.experiment_csv_data:
                st.download_button("📥 点击下载 CSV 文件", st.session_state.experiment_csv_data, f"SeedAI_{st.session_state.user_id}.csv", "text/csv")
